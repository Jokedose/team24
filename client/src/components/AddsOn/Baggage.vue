<template>
<div>
    <Alert :open="success" topic="แจ้งเตือน" desc="บันทึกข้อมูลสำเร็จ" :callback="()=>this.success = false" />
                    ...
                    <div class="row">
                        <div class="col-sm">
                            ...
                            <select v-model="form.baggageTypeId" class="browser-default custom-select">
                                <option disabled value="-1">เลือก</option>
                                <option v-for="t in btypename" :key="t.id" :value="t.id">{{t.btypename}}</option>
                            </select>
                            <br />
                            <br />
...
export default {
    ...
    methods: {
        ...
        deleteBag(id) {
            http.delete("http://172.17.0.200/api/adds-on/" + id).then(res => {
                console.log(res.data);
                this.getBaggage();
        ...
        getBaggagetype() {
            http
                .get("http://172.17.0.200/api/baggage-type/")
                .then(bag => {
        ...
        getBaggage() {
            http
                .get(
                    "http://172.17.0.200/api/adds-on/airport/" + this.filterAirportId
                )
                .then(res => {
        ...
        getImage() {
            http
                .get("http://172.17.0.200/api/baggage-image")
                .then(bimage => {
        ...
        getAirport() {
            http
                .get("http://172.17.0.200/api/airport")
                .then(aerodrom => {
